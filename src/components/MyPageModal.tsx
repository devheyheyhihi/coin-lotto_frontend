...
const MyPageModal = ({ onClose, balance, onBalanceUpdate }: MyPageModalProps) => {
    ...
    // --- DATA FETCHING LOGIC RESTORATION ---

    // Fetch game history and room list for History Tab
    useEffect(() => {
    ...
    // Fetch user transaction history for Wallet -> Transactions Tab
    useEffect(() => {
    ...
    // --- JSX RENDERING LOGIC RESTORATION ---
        ...
        <div className="fixed inset-0 bg-[#FFFFFF80] flex justify-center items-center z-50" onClick={onClose}>
            <div className="bg-[linear-gradient(#000000,#1A255D)] rounded-3xl w-full max-w-2xl text-left shadow-xl flex flex-col" onClick={(e) => e.stopPropagation()}>
                {/* Header and Main Tabs */}
                 <div className='rounded-t-3xl flex justify-between items-center p-4 border-b border-gray-700 bg-gradient-to-r from-[#143892] to-[#0E369D] shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]'>
                            ...
                            : filteredHistory.length > 0 ? (
                                <ul className="space-y-3 pr-2">
                                    {filteredHistory.map((item, index) => (
                                        <li key={index} className={`p-4 rounded-md flex justify-between items-center ${item.wasWinner ? 'bg-[#FFE100] text-black' : 'bg-gray-800/50'}`}>
                                            <div className="flex items-center gap-4">
                                                <span className="bg-black text-white text-center font-bold rounded-md px-3 py-2">Round #{item.roundNumber}</span>
                                                <p className="font-semibold">room _ {item.roomName}</p>
                                            </div>
                                    ...
                                    {isLoadingTransactions ? (
                                        ...
                                        transactions.map((tx) => (
                                            <div key={tx.id} className="p-3 rounded-md bg-gray-800/50 flex justify-between items-center">
                                                <div>
                                                    <p className="font-bold capitalize text-white">{tx.type.toLowerCase().replace('_', ' ')}</p>
